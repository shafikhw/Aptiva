"""
LangGraph-based agentic real estate assistant for the Lebanon-local agency
"Masharee3 Trablos". This variant only uses the local listings stored in
``all_listings.json`` and never reaches out to external listing sources.
"""
...
from . import lease_drafter, schedular
...
SCRAPER_OUTPUT_PATH = "all_listings.json"
SCRAPER_OUTPUT_FILE = Path(__file__).resolve().parent / SCRAPER_OUTPUT_PATH
FALLBACK_LISTINGS_PATH = SCRAPER_OUTPUT_FILE
CONTACT_INFO_FILE = Path(__file__).resolve().parent / "contact_info.json"
THUMBNAIL_WIDTH_PX = 280
MAX_DEFAULT_MOVE_IN_DATE = date(2026, 3, 31)

DOMAIN_GUARD_SYSTEM_MESSAGE = (
    "You are a specialized Lebanese residential real estate assistant for the local agency 'Masharee3 Trablos'. "
    "You only discuss topics directly related to finding and comparing rental properties in Lebanon, "
    "property details, neighborhoods, amenities, scheduling in-person viewings, and lease drafting for this agency's listings. "
    "You do not provide legal or financial advice; remind renters that any lease you draft must be reviewed by a qualified human professional or agent. "
    "Note that the local listing data you reference can be stale or incomplete—encourage users to verify pricing, availability, and terms directly with the agency. "
    "All listings come from the provided local JSON files; do not reference U.S. markets or external sources. "
    "If the user asks for anything outside this scope (coding, politics, medical advice, unrelated travel, etc.), "
    "politely refuse and invite a housing-related question instead. "
    "If details are not available in the provided listings, direct the user to contact the agency using contact_info.json."
)

OFF_TOPIC_REFUSAL = (
    "I can only help with Masharee3 Trablos rentals in Lebanon - matching listings, scheduling viewings, and lease steps. "
    "Ask me about homes in Lebanon or the listings we have locally."
)
...
def _load_local_listings() -> List[Dict[str, Any]]:
    """Load the agency's listings from all_listings.json."""
    if not SCRAPER_OUTPUT_FILE.exists():
        return []
    try:
        raw = json.loads(SCRAPER_OUTPUT_FILE.read_text(encoding="utf-8"))
    except Exception:
        return []
    ...
    for entry in raw:
        ...
        if isinstance(entry, dict) and isinstance(entry.get("data"), dict):
            listing = dict(entry["data"])
            listing.setdefault("_source_id", entry.get("id"))
        elif isinstance(entry, dict):
            listing = dict(entry)
        if listing:
            items.append(listing)
    return validate_listings(items)


def _filter_local_listings(listings: List[Dict[str, Any]], prefs: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Score and sort local listings based on the user's preferences."""
    scored: List[Tuple[float, Dict[str, Any]]] = []
    city_pref = (prefs.get("city") or "").lower()
    area_pref = (prefs.get("area") or prefs.get("neighborhood") or "").lower()
    prop_type_pref = (prefs.get("property_type") or "").lower()

    for listing in listings:
        score = _score_listing(listing, prefs)
        location_label = (_derive_listing_location(listing, prefs) or "").lower()
        if city_pref and city_pref not in location_label:
            score -= 1
        if area_pref and area_pref not in location_label:
            score -= 0.5
        if prop_type_pref:
            listing_type = str(listing.get("property_type") or listing.get("type") or "").lower()
            if prop_type_pref in listing_type:
                score += 1
            else:
                score -= 0.25
    ...
    return [item for _, item in scored if item]


def _contact_info_footer() -> str:
    """Return a short contact line for the agency."""
    try:
        data = json.loads(CONTACT_INFO_FILE.read_text(encoding="utf-8"))
    except Exception:
        return ""
    agency = data.get("agency_name") or "Masharee3 Trablos"
    ...
    if not parts:
        return ""
    return "For detailed questions, contact " + " | ".join(parts) + "."


def is_real_estate_related(user_message: str) -> bool:
    """
    Return True if the message is about residential real estate renting in Lebanon,
...
def compute_missing_preferences(prefs: Dict[str, Any]) -> List[str]:
    ...
    Location and at least one of price/beds/baths are expected.
    """
    missing: List[str] = []
    city = prefs.get("city") or prefs.get("area") or prefs.get("neighborhood")
    if not city:
        missing.append("city or neighborhood in Lebanon (e.g., Tripoli, Dam W Farez)")
...
def generate_persona_reply(
    ...
    safety_instructions = (
        "You are acting as a Lebanese rental agent for the local agency Masharee3 Trablos. "
        "Only use the listing data provided in context (from all_listings.json). "
        "Do NOT invent details; when information is missing, say so and suggest contacting the agency. "
        "Keep responses concise, friendly, and aligned with your persona style. "
        "Show exactly one listing at a time with a clickable title if a URL exists, plus short bullets for location, price, beds, baths, amenities, and why it matches. "
        "If rendered_listings_md is provided in the payload, include that block verbatim. "
        "Remind users they can schedule a viewing that respects Monday-Friday 9:00-17:00 hours, or ask to see another listing."
    )
...
def build_query_from_preferences(prefs: Dict[str, Any]) -> Dict[str, Any]:
    """
    System 2 does not build external URLs; this returns a normalized copy of
    the preferences so downstream nodes can operate on a consistent shape.
    """
    normalized = _normalize_city_state(prefs)
    normalized["property_type"] = (normalized.get("property_type") or "").lower() or None
    normalized["min_beds"] = _safe_int(normalized.get("min_beds"))
    normalized["max_beds"] = _safe_int(normalized.get("max_beds"))
    normalized["min_baths"] = _safe_float(normalized.get("min_baths"))
    normalized["max_baths"] = _safe_float(normalized.get("max_baths"))
    normalized["min_rent"] = _safe_int(normalized.get("min_rent"))
    normalized["max_rent"] = _safe_int(normalized.get("max_rent"))
    return normalized
...
def analyze_preferences(state: AgentState) -> AgentState:
    ...
    system_prompt = (
        "You are a friendly Lebanese rental agent for Masharee3 Trablos. "
        "Extract the user's preferences for local listings (Lebanon only) based solely on their message. "
        ...
        "Allowed preference keys: city, area, neighborhood, location, property_type, min_rent, max_rent, "
        "min_beds, max_beds, min_baths, max_baths, furnished, pet_friendly, amenity_slugs, keyword, tenant_name."
    )
...
def build_query_node(state: AgentState) -> AgentState:
    """Normalize preferences before running a local search."""
    prefs = state.get("preferences", {})
    if state.get("search_query") and not state.get("preferences_updated"):
        return state
    normalized = build_query_from_preferences(prefs)
    return {
        **state,
...
def scrape_listings(state: AgentState) -> AgentState:
    """Load local listings from all_listings.json and filter based on preferences."""
    if state.get("listings") and not state.get("preferences_updated"):
        return state
    prefs = state.get("preferences", {})
    listings = _load_local_listings()
    if not listings:
        reply, persona_key, streamed = generate_persona_reply(
            state,
            intent="error",
            notes="I couldn't load the local listings file. Please try again or contact the agency directly.",
        )
        footer = _contact_info_footer()
        if footer:
            reply = f"{reply}\n\n{footer}"
        return {
            **state,
    ...
    filtered = _filter_local_listings(listings, prefs)
    if not filtered:
        fallback = _load_fallback_listings()
        if fallback:
            filtered = _filter_local_listings(fallback, prefs)
    if not filtered:
        reply, persona_key, streamed = generate_persona_reply(
            state,
            intent="no_results",
            notes="No listings were found in the local catalog; ask to adjust location, price, or beds/baths.",
        )
        footer = _contact_info_footer()
        if footer:
            reply = f"{reply}\n\n{footer}"
    ...
    return {**state, "listings": filtered, "scraped_listings": filtered, "preferences_updated": False}
...
def rank_and_format(state: AgentState) -> AgentState:
    """Score listings, select one option, and craft the user-facing reply."""
    prefs = state.get("preferences", {})
    listings = state.get("enriched_listings") or state.get("scraped_listings") or state.get("listings") or []
    ranked = sorted(listings, key=lambda item: _score_listing(item, prefs), reverse=True)
    if not ranked:
        reply, persona_key, streamed = generate_persona_reply(
            state,
            intent="no_results",
    ...
    listing_offset = max(0, min(state.get("listing_offset", 0), len(ranked) - 1))
    primary_listing = ranked[listing_offset]
    listing_summaries: List[Dict[str, Any]] = []
    view = _listing_prompt_view(primary_listing, prefs)
    view["rank"] = listing_offset + 1
    view["score"] = _score_listing(primary_listing, prefs)
    view["why_match"] = _reason_tags(primary_listing, prefs, view.get("amenities", []))
    listing_summaries.append(view)

    notes = (
        "Present a single listing (no more than one). Offer to show another option, tweak preferences, "
        "or schedule a viewing that respects Monday-Friday 9:00-17:00 local hours."
    )
    reply, persona_key, streamed = generate_persona_reply(
    ...
    return {
        **state,
        ...
        "focused_listing": primary_listing,
        "reply_streamed": streamed,
    }
...
def _extract_price_range(listing: Dict[str, Any]) -> Tuple[Optional[int], Optional[int]]:
    ...
    if not prices:
        ...
        if isinstance(listing.get("price"), dict):
            raw_price = listing["price"].get("amount") or listing["price"].get("value")
        elif listing.get("price"):
            raw_price = listing.get("price")
        if isinstance(raw_price, str):
            matches = re.findall(r"\d+", raw_price.replace(",", ""))
            prices.extend(int(m) for m in matches)
        elif isinstance(raw_price, (int, float)):
            prices.append(int(raw_price))
    if not prices:
        return None, None
...
def _extract_beds_baths(listing: Dict[str, Any]) -> Tuple[Optional[float], Optional[float]]:
    ...
    if beds is None:
        try:
            beds = float(listing.get("beds"))
        except Exception:
            beds = beds
    if baths is None:
        try:
            baths = float(listing.get("baths"))
        except Exception:
            baths = baths
    return beds, baths
...
def _answer_property_question(state: AgentState, question: Optional[str], option_index: Optional[int]) -> str:
    ...
    system_prompt = (
        "You are a Lebanese rental real estate assistant for Masharee3 Trablos. "
        "Answer the question about the property using the provided listing summary. "
        "If the detail is missing, admit it briefly and suggest contacting the agency for specifics. "
        "Keep responses under six sentences."
    )
...
def _wants_viewing(text: str) -> bool:
    ...
    return any(keyword in lowered for keyword in SCHEDULING_KEYWORDS)


def _parse_slot_choice(user_input: str, slots: List[Dict[str, str]]) -> Optional[int]:
    ...
    for idx, slot in enumerate(slots):
        if slot.get("label") and slot["label"].lower() in lowered:
            return idx
    return None


def _infer_requested_window(user_input: str) -> Optional[Tuple[str, str, str]]:
    """Parse a requested weekday (e.g., 'next Monday') into a 1-day window."""
    lowered = (user_input or "").lower()
    if not lowered:
        return None
    now_local = datetime.now(schedular.LOCAL_TZ)
    ...
    if "tomorrow" in lowered:
        ...
        for alias, idx in weekday_map.items():
    ...
    start_iso = start_local.astimezone(timezone.utc).isoformat(timespec="minutes")
    end_iso = end_local.astimezone(timezone.utc).isoformat(timespec="minutes")
...
def _propose_slots(
    ...
    listing: Dict[str, Any],
    window_days: int = 3,
    target_window: Optional[Tuple[str, str, str]] = None,
) -> str:
    if target_window:
        window_start_iso, window_end_iso, window_label = target_window
    else:
        now = datetime.now(timezone.utc)
        window_start_iso = now.isoformat(timespec="minutes")
        window_end_iso = (now + timedelta(days=window_days)).isoformat(timespec="minutes")
        window_label = None
    try:
        slots = asyncio.run(
            schedular.backend_get_common_slots(
    ...
    if not slots:
        footer = _contact_info_footer()
        label_text = f" for {window_label}" if window_label else " in the next few days"
        follow_up = "Share a day/time that works for you (Mon-Fri, 9:00-17:00), and I'll check availability."
        if footer:
            follow_up += f" {footer}"
        return f"I couldn't find open slots{label_text}. {follow_up}"

    header = "Here are the available 30-minute slots (Mon-Fri, 9:00-17:00"
    if window_label:
        header += f" for {window_label}"
    header += "):"
    lines = [header]
    for idx, slot in enumerate(slots, start=1):
        lines.append(f"{idx}. {slot['label']}")
    lines.append("Pick a slot number or share another time that fits you.")
    return "\\n".join(lines)


def _book_slot(state: AgentState, slot: Dict[str, str]) -> str:
    scheduling_state = state.get("scheduling") or {}
    state.pop("pending_tour_confirmation", None)
    listing = scheduling_state.get("listing") or state.get("focused_listing")
    if not listing:
        return "Please pick a listing first, then I can book a viewing."
    title = (listing.get("about") or {}).get("title") or listing.get("id") or "Apartment viewing"
    address = _derive_listing_address(listing) or _derive_listing_location(listing, state.get("preferences", {})) or "Lebanon"
    try:
        result = asyncio.run(
            schedular.backend_book_tour(
                slot_start_iso=slot["start"],
                slot_end_iso=slot["end"],
                listing_title=title,
        ...
        footer = _contact_info_footer()
        failure = f"I couldn't book that slot ({exc})."
        return f"{failure} Please share another time or contact the agency directly. {footer}" if footer else failure

    state["scheduling"] = {}
    state["last_scheduled_slot"] = slot
    state["last_listing"] = listing
    state["focused_listing"] = listing
    label = slot.get("label") or f"{slot.get('start')} -> {slot.get('end')}"
    confirmation = f"You're booked for a viewing: {label} at {address}."
    return f"{confirmation}\nAfter the viewing, I can draft the lease for you - want me to start that now?"


def _list_upcoming_tours() -> str:
    try:
        tours = asyncio.run(schedular.backend_list_tours())
    except Exception:
        return "I couldn't load the calendars right now. Please try again or share a preferred time."
    if not tours:
        return "No upcoming tours are on the calendar. Want me to schedule one?"
    lines = ["Upcoming tours:"]
    for tour in tours:
        label = schedular.fmt_iso_pretty(tour["start"]) if tour.get("start") else "Time not listed"
        lines.append(f"- {tour.get('summary', 'Tour')} ({tour.get('calendar')}) at {label}")
    return "\n".join(lines)


def _build_scheduler_prompt(listing: Optional[Dict[str, Any]]) -> str:
    """System prompt mirroring schedular.py but enriched with the focused listing."""
    now_utc = datetime.now(timezone.utc).isoformat(timespec="seconds")
    now_local = datetime.now(schedular.LOCAL_TZ).isoformat(timespec="seconds")
    title = (listing or {}).get("about", {}).get("title") or (listing or {}).get("id") or schedular.LISTING_TITLE
    address = (
        _derive_listing_address(listing)
        or _derive_listing_location(listing or {}, {})
        or schedular.LISTING_ADDRESS
    )
    return (
        "You are an apartment tour scheduling assistant.\n"
        f"Current UTC time is {now_utc}.\n"
        f"Current local time (offset {schedular.LOCAL_TZ_OFFSET_HOURS:+g}) is {now_local}.\n\n"
        f"The current apartment listing is '{title}' at '{address}'.\n"
        f"The renter's email is {schedular.USER_EMAIL} with calendar {schedular.USER_CALENDAR_ID}.\n"
        f"The landlord calendar is {schedular.LANDLORD_CALENDAR_ID}.\n\n"
        "Your job:\n"
        ...
        "- Each slot from get_common_slots has a 'label' field. Always show that label exactly as given (do NOT recompute or change the date/time yourself).\n"
        "- Before calling get_common_slots, acknowledge the day/date and tell the user you'll check availability (e.g., 'Please hold on while I check the available slots'). Stream that acknowledgement promptly, then share the slots once the tool returns.\n"
        "- Be clear and polite, and confirm actions.\n"
    )


def _listing_title_and_address(listing: Optional[Dict[str, Any]]) -> Tuple[str, str]:
    title = (listing or {}).get("about", {}).get("title") or (listing or {}).get("id") or "Apartment viewing"
    address = _derive_listing_address(listing) or _derive_listing_location(listing or {}, {}) or "Lebanon"
    return title, address


def _wants_new_listing(text: str) -> bool:
    lowered = (text or "").lower()
    keywords = (
        "another",
        "different",
...
def _load_fallback_listings(limit: int = 50) -> List[Dict[str, Any]]:
    return _load_local_listings()[:limit]
...
def answer_with_existing(state: AgentState) -> AgentState:
    ...
    if not listings:
        reply, persona_key, streamed = generate_persona_reply(
            state,
            intent="no_cached",
            notes="No cached listings yet; ask for city/area/budget to start a search.",
        )
        return {**state, "reply": reply, "active_persona": persona_key, "reply_streamed": streamed}

    offset = max(0, min(state.get("listing_offset", 0), len(listings) - 1))
    preview = listings[offset]
    listing_summaries: List[Dict[str, Any]] = []
    view = _listing_prompt_view(preview, prefs)
    view["rank"] = offset + 1
    view["why_match"] = _reason_tags(preview, prefs, view.get("amenities", []))
    listing_summaries.append(view)
    notes = (
        "Answer the user's follow-up using the current listing; remind them you can show the next option or adjust preferences."
    )
    reply, persona_key, streamed = generate_persona_reply(
...
class System2Agent:
    """Local Lebanese real estate agent with scheduling and lease drafting support."""

    def __init__(self, state: Optional[AgentState] = None) -> None:
        self._app = build_graph()
        if state:
            restored: AgentState = copy.deepcopy(state)
            restored.setdefault("messages", [])
            restored.setdefault("preferences", {})
            restored.setdefault("persona_mode", DEFAULT_PERSONA_MODE)
            restored.setdefault("preferences_updated", False)
            restored.setdefault("listing_offset", 0)
            self.state = restored
        else:
            self.state = {
                "messages": [],
    ...
    def _scheduler_key(self, listing: Optional[Dict[str, Any]]) -> str:
        title = (listing or {}).get("about", {}).get("title") or (listing or {}).get("id") or ""
        address = _derive_listing_address(listing) or _derive_listing_location(listing or {}, {}) or ""
        return f"{title}|{address}"

    def _ensure_scheduler_thread(self, listing: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        key = self._scheduler_key(listing)
        existing_key = self.state.get("scheduler_context_key")
        if self.state.get("scheduler_thread") and key == existing_key:
            return self.state["scheduler_thread"]
        prompt = _build_scheduler_prompt(listing)
        thread = [{"role": "system", "content": prompt}]
        self.state["scheduler_thread"] = thread
        self.state["scheduler_context_key"] = key
        return thread

    def _should_route_to_scheduler(self, text: str) -> bool:
        lowered = (text or "").lower()
        # Do not hijack explicit lease/contract requests into scheduling.
        if "lease" in lowered or "contract" in lowered:
            return False
        if self.state.get("scheduler_thread"):
            return True
        scheduling_terms = (
            "schedule",
            "tour",
        ...
        return any(term in lowered for term in scheduling_terms)

    def _run_scheduler_turn(self, user_input: str, stream_handler: Optional[Callable[[str], None]] = None) -> str:
        """
        Mirror schedular.py conversation loop with tool calls, keeping messages in state.
        """
        listing = self.state.get("focused_listing")
        thread = self._ensure_scheduler_thread(listing)
        thread.append({"role": "user", "content": user_input})
        self.state["scheduler_thread"] = thread
        stream_cb = stream_handler or STREAM_CALLBACK.get()

        for _ in range(4):
            try:
                msg, _ = schedular.stream_completion_with_tools(
                    model=schedular.OPENAI_MODEL,
                    messages=thread,
                    tools=schedular.TOOLS,
                    tool_choice="auto",
                    stream_to_stdout=stream_cb is None,
                    stream_callback=stream_cb,
                )
            except Exception as exc:
                fallback = (
                    "Scheduling is temporarily unavailable (calendar service error). "
                    "Share a couple of preferred times and I'll notify the property team manually."
                )
                logger.warning("scheduler_unavailable", extra={"conversation_id": self.state.get("conversation_id"), "error": str(exc)})
            ...
            if tool_calls:
                ...
                for tool_call in tool_calls:
                    if isinstance(tool_call, dict):
                        normalized_calls.append(dict(tool_call))
                    else:
                ...
                for tool_call in normalized_calls:
                    normalized_tc: Dict[str, Any] = dict(tool_call)
                    func = normalized_tc.get("function") or {}
                    try:
                    ...
                    if tool_name == "book_tour":
                        slot = {
                            "start": args.get("slot_start_iso"),
                            "end": args.get("slot_end_iso"),
                        }
                        slot_label = args.get("slot_label") or args.get("label")
                        if slot.get("start") and slot.get("end") and not slot_label:
                            try:
                                slot_label = f"{schedular.fmt_iso_pretty(slot['start'])} -> {schedular.fmt_iso_pretty(slot['end'])}"
                            except Exception:
                                slot_label = f"{slot.get('start')} -> {slot.get('end')}"
                        slot["label"] = slot_label
                        listing_title, listing_address = _listing_title_and_address(listing) if listing else ("", "")
                        address = args.get("listing_address") or listing_address or ""
                        confirmation_text = (
                            f"Ready to book {slot_label or 'that slot'}"
                            + (f" at {address}" if address else "")
                            + ". Type CONFIRM to place it on both calendars, or share another time."
                        )
                        self.state["pending_tour_confirmation"] = {
                            "slot": slot,
                            "listing": listing,
                            "label": slot_label,
                            "address": address,
                        ...
                        self.state["scheduling"] = {
                            "stage": "await_slot_confirmation",
                            "slots": [slot],
                            "listing": listing,
                        }
                        thread[-1]["content"] = confirmation_text
                        thread[-1]["tool_calls"] = []
                        self.state["scheduler_thread"] = thread
                        return confirmation_text
                    if tool_name == "book_tour" and listing:
                        title, address = _listing_title_and_address(listing)
                        args.setdefault("listing_title", title)
                        args.setdefault("listing_address", address)
                        func["arguments"] = json.dumps(args)
                        normalized_tc["function"] = func

                    result = asyncio.run(schedular.handle_tool_call(normalized_tc))
                    tc_id = normalized_tc.get("id")
                    tc_func = normalized_tc.get("function", {})
                    tool_name = tc_func.get("name") if isinstance(tc_func, dict) else None
                    thread.append(
                        {
                            "role": "tool",
                            "tool_call_id": tc_id,
            ...
            final_text = msg.get("content") or "I couldn't complete that scheduling request. Could you rephrase?"
            thread.append({"role": "assistant", "content": final_text})
            self.state["scheduler_thread"] = thread
            return final_text

        fallback = "Sorry, scheduling ran into an issue. Please try again or share a different time."
        thread.append({"role": "assistant", "content": fallback})
        self.state["scheduler_thread"] = thread
        return fallback

    def _advance_scheduling(self, user_input: str) -> str:
        scheduling = self.state.get("scheduling") or {}
        listing = scheduling.get("listing") or self.state.get("focused_listing")
        normalized = (user_input or "").strip().lower()
        pending_tour = self.state.get("pending_tour_confirmation")
        if pending_tour:
            if normalized in {"confirm", "confirm tour", "confirm booking"}:
                slot = pending_tour.get("slot")
                self.state.pop("pending_tour_confirmation", None)
                if slot:
                    return _book_slot(self.state, slot)
                return "I lost the slot details. Please pick a time again."
            if normalized in {"cancel", "cancel tour", "restart", "start over", "back"}:
                self.state.pop("pending_tour_confirmation", None)
                return "No problem—tell me another time that works and I'll check availability."
            if any(keyword in normalized for keyword in ("change", "different", "another", "other", "resched", "reschedule")):
                self.state.pop("pending_tour_confirmation", None)
            else:
                label = pending_tour.get("label") or (pending_tour.get("slot") or {}).get("label")
                return f"I'm ready to book {label or 'that slot'}. Type CONFIRM to proceed or share a different time."
        custom_window = _infer_requested_window(user_input)
        if custom_window and listing:
            return _propose_slots(self.state, listing, target_window=custom_window)
        if scheduling.get("stage") == "await_slot_choice" and scheduling.get("slots"):
            idx = _parse_slot_choice(user_input, scheduling["slots"])
            if idx is not None:
                chosen_slot = scheduling["slots"][idx]
                address = _derive_listing_address(listing) or _derive_listing_location(listing or {}, {}) or "the property"
                label = chosen_slot.get("label") or f"{chosen_slot.get('start')} -> {chosen_slot.get('end')}"
                self.state["pending_tour_confirmation"] = {
                    "slot": chosen_slot,
                    "listing": listing,
                    "label": label,
                ...
                scheduling["stage"] = "await_slot_confirmation"
                scheduling["listing"] = listing
                self.state["scheduling"] = scheduling
                return (
                    f"Ready to book {label} at {address}. "
                    "Type CONFIRM to place it on both calendars, or share another time."
                )
            window_days = scheduling.get("window_days", 3) + 2
            return _propose_slots(self.state, listing, window_days=window_days) if listing else "Tell me which listing you want to view."
        if listing:
            return _propose_slots(self.state, listing)
        return "Tell me which listing you'd like to view, and I'll suggest times within Monday-Friday 9:00-17:00."

    def handle_message(self, text: str, stream_handler: Optional[Callable[[str], None]] = None) -> str:
        ...
        if command_reply:
            ...
            self._append_message("assistant", command_reply)
            return command_reply

        if not is_real_estate_related(user_input):
            refusal = OFF_TOPIC_REFUSAL
            self._append_message("user", user_input)
            self._append_message("assistant", refusal)
        ...
        if injection_reason:
            ...
            # Keep the reminder in the thread for the model.
            if not any(msg.get("content") == INJECTION_REMINDER for msg in self.state.get("messages", []) if msg.get("role") == "system"):
                self._append_message("system", INJECTION_REMINDER)
        lower = user_input.lower()
        lease_intent = "lease" in lower or "contract" in lower
        if lease_intent:
            # Allow lease drafting to proceed even if a scheduler session was started earlier.
            self.state.pop("scheduler_thread", None)
            self.state.pop("scheduler_context_key", None)
            self.state.pop("scheduling", None)
            self.state.pop("pending_tour_confirmation", None)

        # New scheduler agent (tool-calling) path
        if not lease_intent and not self.state.get("scheduling") and self._should_route_to_scheduler(user_input):
            reply = self._run_scheduler_turn(user_input, stream_handler=stream_handler)
            self._append_message("assistant", reply)
            return reply

        # Scheduling flow first
        if self.state.get("scheduling") and not lease_intent:
            reply = self._advance_scheduling(user_input)
            self._append_message("assistant", reply)
            return reply

        if "calendar" in lower and ("tour" in lower or "view" in lower or "schedule" in lower):
            reply = _list_upcoming_tours()
            self._append_message("assistant", reply)
            return reply

        if _wants_viewing(user_input) and self.state.get("focused_listing"):
            custom_window = _infer_requested_window(user_input)
            reply = _propose_slots(
                self.state,
        ...
        if _wants_new_listing(user_input) or (user_input.lower() in SHORT_NEGATIVE_RESPONSES and (self.state.get("ranked_listings") or self.state.get("listings"))):
            total = len(self.state.get("ranked_listings") or self.state.get("listings") or [])
            if total:
                self.state["listing_offset"] = (self.state.get("listing_offset", 0) + 1) % total
                token = None
                if stream_handler:
                    token = STREAM_CALLBACK.set(stream_handler)
                try:
                    self.state = rank_and_format(self.state)
                finally:
                    if token is not None:
                        STREAM_CALLBACK.reset(token)
                reply = self.state.get("reply") or "Here's another option."
                if not self.state.get("reply_streamed"):
                    self._append_message("assistant", reply)
                else:
        ...
        if self.state.get("lease_collection"):
            ...
            if follow_up:
                self._append_message("assistant", follow_up)
                return follow_up

        update_reply = handle_lease_update_request(self.state, user_input)
        if update_reply:
            self._append_message("assistant", update_reply)
            return update_reply

        lease_reply = handle_lease_command(self.state, user_input)
        if lease_reply:
            self._append_message("assistant", lease_reply)
            return lease_reply
...
def main() -> None:
    """Simple REPL interface for the Lebanon agent."""
    try:
        get_openai_client()
    ...
    agent = System2Agent()
    print("Hi! I'm the Masharee3 Trablos rental assistant. Tell me your city/area, budget, beds/baths, and must-haves.")
    while True:
        try:
